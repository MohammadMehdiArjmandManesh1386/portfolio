...
function generateExportedJsCode() {
  ...
  const fullCode = `// اطلاعات اولیه بیوگرافی محمد مهدی ارجمند منش
...
function generateExportedJsCode() {
  ...
  document.getElementById('exportedJsTextarea').value = \`// اطلاعات همگانی بیوگرافی محمد مهدی ارجمند منش
const defaultBio = \${bioStr};

// اطلاعات همگانی نمونه‌کارها
const defaultProjects = \${projStr};\`;

  document.getElementById('exportCodeContainer').classList.remove('d-none');
}
...
function copyExportedJsCode() {
  ...
  alert('✅ کد دو متغیر اولیه کپی شد!');
}
...
`;
  ...
  document.getElementById('exportedJsTextarea').value = fullCode;
  document.getElementById('exportCodeContainer').classList.remove('d-none');
}
...
function copyExportedJsCode() {
  ...
  textarea.select();
  document.execCommand('copy');
  alert('✅ کد app.js کپی شد! حالا فایل app.js در گیت‌هاب را باز کرده، این کد را جایگزین و Save کنید تا در تمام گوشی‌ها لود شود.');
}
